package com.walter.africansayings;
...
public class SQLiteHandler extends SQLiteOpenHelper {
 ...
 private static final String MYDATABASE = "appdata.db";
...
//  onCreate(db);
 ...
 public void Save(String data,String comment){
	 ...
	 public ArrayList<String> search(String search_term)
	 {
		 ...
		 SQLiteDatabase db = this.getReadableDatabase();
		 //use select query to search data in table - save found record in a cursor
		 final Cursor c = db.rawQuery("SELECT Data FROM " +	SAMPLE_TABLE_NAME +" WHERE Data LIKE '%"+search_term+"%'  LIMIT 15", null);	 
		 //count number of rows returned by the cursor
		 int count = c.getCount();	 
		 //if its zero, no records found
		 ArrayList<String> list=new ArrayList<String>();
