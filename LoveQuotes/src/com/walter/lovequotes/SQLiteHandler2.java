...
public class SQLiteHandler2 extends SQLiteOpenHelper {
 ...
 private static final String MYDATABASE = "lovefav.db";
 private static final int VERSION = 1;//db version
 private final String SAMPLE_TABLE_NAME = "fav";
...
//  onCreate(db);
 ...
 public void Save(MessageDetails detail){
 
	 SQLiteDatabase db = this.getWritableDatabase();
	 //content values hold the collumns & their values
	 ContentValues c = new ContentValues();	 
	 c.put("Data", detail.getDesc());
	 c.put("Comment", detail.getSub());
	 ...
	 public ArrayList<MyQuotes> search(String search_term)
	 {
		 ...
		 SQLiteDatabase db = this.getReadableDatabase();
		 //use select query to search data in table - save found record in a cursor
		 final Cursor c = db.rawQuery("SELECT * FROM " +	SAMPLE_TABLE_NAME +" WHERE Data LIKE '%"+search_term+"%'  LIMIT 30", null);	 
		 //count number of rows returned by the cursor
		 int count = c.getCount();	 
		 //if its zero, no records found
		 ArrayList<MyQuotes> list=new ArrayList<MyQuotes>();
