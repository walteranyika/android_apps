...
public class SQLiteHandler extends SQLiteOpenHelper {
 ...
 private static final String MYDATABASE = "tapcount.db";
 private static final int VERSION = 1;//db version
 private final String SAMPLE_TABLE_NAME = "counts";
...
//  onCreate(db);
 ...
 //method to receive our 3 Strings
	 ...
	 public ArrayList<MyQuotes> getDataa()
	 {
		 ...
		 SQLiteDatabase db = this.getReadableDatabase();
		 //use select query to search data in table - save found record in a cursor
		 final Cursor c = db.rawQuery("SELECT * FROM " +	SAMPLE_TABLE_NAME +" ORDER BY Id DESC", null);	 
		 //count number of rows returned by the cursor
		 int count = c.getCount();	 
		 //if its zero, no records found
	 ...
	 public void delete(String data,String count)
	 {
		 SQLiteDatabase db = this.getReadableDatabase();
		 String whereClause="Data='"+count+"'  AND Comment='"+data+"'";
		 db.delete(SAMPLE_TABLE_NAME, whereClause, null);
		 db.close();
	 }
}
